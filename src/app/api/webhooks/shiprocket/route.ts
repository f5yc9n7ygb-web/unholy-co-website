 ...
 *
 * Receives shipment status updates from Shiprocket and updates Airtable.
 ...
 * Empty/missing payloads are accepted without auth for Shiprocket's test pings.
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (records.length === 0) {
      console.warn(`Shiprocket webhook: No payment record found for ${razorpayOrderId || awb}`)
    ...
    // Notify on: Shipped(6), In Transit(18), Out for Delivery(17), Delivered(7)
    ...
    if (shouldNotify && customerEmail) {
      ...
      }).catch((err) => console.error("Shiprocket webhook: email failed:", err))
    }
...
export async function GET() {
  return NextResponse.json({
    ok: true,
    message: "Shiprocket webhook endpoint is live. Send POST requests for shipment updates.",
  })
}
