 ...
 * Carries the live dispatch countdown so the honest-urgency clock rides at the
 * thumb for the whole scroll, not just where the offer blocks happen to be.
 */
export function SinStickyBar({
  ...
  return (
    <div
      aria-hidden={!visible}
      className={`fixed inset-x-0 bottom-0 z-[55] border-t border-bone/15 bg-black/85 backdrop-blur-md transition-transform duration-300 ease-out md:hidden ${
        visible ? "translate-y-0" : "translate-y-full"
      }`}
      style={{ paddingBottom: "max(0.75rem, env(safe-area-inset-bottom))" }}
    >
      {/* live courier-cutoff seam — honest urgency riding at the thumb */}
      <div className="flex justify-center border-b border-bone/10 px-4 py-1.5">
        <SinDispatch compact />
      </div>
